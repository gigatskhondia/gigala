...
class Actor(nn.Module):
    ...
    def forward(self, state, goal):
        # print(state, goal,self.action_bounds,self.offset.shape)
...
class DDPG:
    def __init__(self, state_dim, action_dim, action_bounds, offset, lr, H, optimizer,layer_dim,n_layers):
        ...
        self.actor = Actor(state_dim, action_dim, action_bounds, offset, layer_dim,n_layers).to(device)
        self.actor_optimizer=getattr(optim, optimizer)(self.actor.parameters(), lr= lr)
        self.critic = Critic(state_dim, action_dim, H, layer_dim, n_layers).to(device)
