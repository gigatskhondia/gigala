...
class HAC:
    def __init__(self, k_level, H, state_dim, action_dim, render, threshold, 
                 action_bounds, action_offset, state_bounds, state_offset, lr,optimizer,layer_dim, n_layers):
        
        # adding lowest level
        self.HAC = [DDPG(state_dim, action_dim, action_bounds, action_offset, lr, H,optimizer,layer_dim, n_layers)]
        self.replay_buffer = [ReplayBuffer()]
        
        # adding remaining levels
        for _ in range(k_level-1):
            self.HAC.append(DDPG(state_dim, state_dim, state_bounds, state_offset, lr, H, optimizer,layer_dim, n_layers))
    ...
    def check_goal(self, state, goal, threshold, env):
        t, c = fast_stopt(get_args(*mbb_beam(rd=env.rd)), state)
        if abs(c-goal[0]) > threshold[0] or abs(t-goal[1]) > threshold[1]:
            return False
        return True
    ...
    def run_HAC(self, env, i_level, state, goal, is_subgoal_test):
        ...
        for _ in range(self.H):
            ...
            if i_level > 0:
                ...
                if is_next_subgoal_test and not self.check_goal(action, next_state, self.threshold,env):
            ...
            goal_achieved = self.check_goal(next_state, goal, self.threshold,env)
        ...
        goal_transitions[-1][2] = 0.0 # self.reward
    ...
    def update(self, n_iter, batch_size,env):
        for i in range(self.k_level):
            self.HAC[i].update(self.replay_buffer[i], n_iter, batch_size,env)
