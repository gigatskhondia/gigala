...
class CantileverEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.N_DISCRETE_ACTIONS=4*4 # optimize code - make more generic
        ...
        self.M=Model((4, 4)) # optimize code - make more generic
    ...
    def step(self, action):
        ...
        action=action*(1-self.penalty_coeff*self.x[self.save_x*4:(self.save_x+1)*4,self.save_y*4:(self.save_y+1)*4].reshape(len(action),)) # optimize code - make more generic
        ...
        if self.level == 1:
            ...
            x_c, y_c=x_c+4*self.save_x, y_c+4*self.save_y # optimize code - make more generic
            print(x_c)
