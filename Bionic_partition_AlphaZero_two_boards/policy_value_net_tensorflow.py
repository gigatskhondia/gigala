...
class PolicyValueNet():
    ...
    def policy_value_fn(self, board):
        ...
        if board.current_player == board.players[0]:
            legal_positions = board.availables1
        else:
            legal_positions = board.availables2
        # print(board.current_state().shape)
        current_state = np.ascontiguousarray(board.current_state().reshape(
                -1, 4, self.board_width, self.board_height))
        # print(current_state)
        act_probs, value = self.policy_value(current_state)

        if board.current_player == board.players[1]:
            legal_positions=[x-board.height*board.width for x in legal_positions]
